...
function lib:init(title, doSplash, toggleKey, deletePrevious, themeName)
    ...
    local WIN_W     = isMobile and 0    or 780
    local WIN_H     = isMobile and 0    or 600
    local WIN_XS    = isMobile and 0.95 or 0
    local WIN_YS    = isMobile and 0.88 or 0
    local SIDEBAR_W = isMobile and 160  or 220
    ...
    if deletePrevious then
        ...
        local existingMT = guiParent:FindFirstChild("AppleLibMobileToggle")
        if existingMT then existingMT:Destroy() end
    end
    ...
    main.Size = UDim2.new(WIN_XS, WIN_W, WIN_YS, WIN_H)
    ...
    sideDivider.Position = UDim2.new(0, SIDEBAR_W, 0, 55)
    ...
    workareaOuter.Position = UDim2.new(0, SIDEBAR_W + 1, 0, 55)
    workareaOuter.Size = UDim2.new(1, -(SIDEBAR_W + 1), 1, -55)
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        task.wait(0.5)
        pcall(function() mobileToggleGui:Destroy() end)
        scrgui:Destroy()
    end)
    ...
    -- ── Mobile Toggle DUI ─────────────────────────────────────────────────────
    -- A floating Apple button pinned to the top-centre of the screen.
    -- Lets mobile users toggle the window without needing a keyboard key.
    -- Draggable so players can reposition it out of the way.
    local mobileToggleGui = Instance.new("ScreenGui")
    mobileToggleGui.Name  = "AppleLibMobileToggle"
    mobileToggleGui.ResetOnSpawn = false
    mobileToggleGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    _protectGui(mobileToggleGui)
    mobileToggleGui.Parent = _getGui()

    local mtFrame = Instance.new("Frame")
    mtFrame.Name = "mtFrame"
    mtFrame.Parent = mobileToggleGui
    mtFrame.AnchorPoint = Vector2.new(0.5, 0)
    mtFrame.BackgroundColor3 = t.Window
    mtFrame.BackgroundTransparency = isMobile and 0.08 or 0.15
    mtFrame.Position = UDim2.new(0.5, 0, 0, 14)
    mtFrame.Size = UDim2.new(0, 54, 0, 54)
    mtFrame.ZIndex = 60
    _makeCorner(mtFrame, 14)
    _makeShadow(mtFrame, 59, UDim2.new(1.35, 0, 1.35, 0), 0.45)
    _makeStroke(mtFrame, t.Separator, 1, 0.2)

    local mtIcon = Instance.new("ImageLabel")
    mtIcon.Parent = mtFrame
    mtIcon.BackgroundTransparency = 1
    mtIcon.AnchorPoint = Vector2.new(0.5, 0.5)
    mtIcon.Position = UDim2.new(0.5, 0, 0.5, 0)
    mtIcon.Size = UDim2.new(0.72, 0, 0.72, 0)
    mtIcon.ZIndex = 61
    mtIcon.Image = "rbxassetid://12621719043"
    mtIcon.ScaleType = Enum.ScaleType.Fit

    local mtBtn = Instance.new("TextButton")
    mtBtn.Parent = mtFrame
    mtBtn.BackgroundTransparency = 1
    mtBtn.Size = UDim2.new(1, 0, 1, 0)
    mtBtn.Text = ""
    mtBtn.ZIndex = 62

    -- Drag support for the mobile toggle
    local mtDragging, mtDragStart, mtStartPos2
    local mtDragInput2

    mtFrame.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1
        or inp.UserInputType == Enum.UserInputType.Touch then
            mtDragging = true
            mtDragStart = inp.Position
            mtStartPos2 = mtFrame.Position
    ...
    mtFrame.InputChanged:Connect(function(inp)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if inp == mtDragInput2 and mtDragging then
            local delta = inp.Position - mtDragStart
            mtFrame.Position = UDim2.new(
                mtStartPos2.X.Scale,
                mtStartPos2.X.Offset + delta.X,
                mtStartPos2.Y.Scale,
                mtStartPos2.Y.Offset + delta.Y
            )
        end
    end)
    ...
    mtBtn.InputEnded:Connect(function(inp)
        ...
        and mtTapStart then
            local dist = (inp.Position - mtTapStart).Magnitude
            if dist < 10 then   -- tiny movement = tap, not drag
                _doToggle()
                -- Pulse feedback
                _tween(mtFrame,
                ...
                task.delay(0.12, function()
                    _tween(mtFrame,
                        ...
                        {BackgroundTransparency = isMobile and 0.08 or 0.15}
                    )
                end)
            end
            mtTapStart = nil
    ...
    -- Track window toggle to update DUI opacity
    local _origDoToggle = _doToggle
    -- (opacity hint: slightly dimmed when window is showing)
    -- Expose destroy on the mobile GUI too
    window._mobileToggleGui = mobileToggleGui
...
function lib:DestroyAll()
    ...
    for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if gui.Name == "AppleLibV1" or gui.Name == "AppleLibToasts"
        or gui.Name == "AppleLibMobileToggle" then
            gui:Destroy()
        end
    end
    if gethui then
...
-- ─────────────────────────────────────────────────────────────────────────────
-- ELEMENT SUMMARY REFERENCE  (for IDE/explorer use)
-- ─────────────────────────────────────────────────────────────────────────────
--[[
    WINDOW METHODS
    ──────────────
    ...
    win:Notify(title, body, btnText, icon, callback)
    win:Notify2(title, body, btn1, btn2, icon, cb1, cb2)
    win:Divider(name)
    win:Section(name)  → sec

    SECTION METHODS  (sec = win:Section("Name"))
    ────────────────
    sec:Select()
    sec:Divider(name)
    sec:Label(name)                           → { Set, Get }
    sec:Button(name, desc?, callback)         → { SetText, SetEnabled }
